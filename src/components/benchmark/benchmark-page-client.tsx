...
export interface BenchmarkPageClientProps {
  ...
  /** Current org metrics (only when opted in). */
  orgMetrics: {
  ...
  /** All aggregate rows from benchmark_aggregates. */
  aggregateRows: Array<{
...
export function BenchmarkPageClient({
  ...
  const metrics: { key: BenchmarkMetricKey; you: number | null; peer: number | null }[] = [
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {metrics.map((m) => (
            <BenchmarkChartCard
              ...
              metricKey={m.key}
              yourValue={m.you}
              peerValue={m.key === 'inspectionScore' ? m.peer : m.peer}
              peerCount={peerCount}
              showBar
            />
          ))}
