...
export default async function ProGearEquipmentPage({
  ...
  const [products, brands] = await Promise.all([
    getFilteredProGearProducts({ ...params, category: 'equipment' }, { category: 'equipment' }),
    getProGearBrands('equipment'),
  ]);
  ...
  return (
    <>
      ...
      <div className="mb-4">
        <StorefrontFilters brands={brands} currentCategory="equipment" />
      </div>
      ...
      {list.length === 0 && (
        <p className="py-8 text-center text-muted-foreground">No equipment match your filters.</p>
      )}
    </>
  );
}
