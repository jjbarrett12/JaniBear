...
export default async function TopTargetsPage() {
  ...
  const byRank = (topTargets || []).reduce<Record<number, (typeof topTargets)[0]>>((acc, row) => {
    acc[row.rank] = row;
    return acc;
  }, {});
  const assignedLeadIds = new Set((topTargets || []).map((t) => t.lead_id));
      ...
      <Card className="dark:bg-gray-800">
        ...
        <CardContent>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            {slots.map((rank) => {
              const target = byRank[rank];
              const lead = target?.leads as typeof leads extends (infer L)[] ? L : never;
              return (
                <TopTargetCard
                  key={rank}
                  rank={rank}
